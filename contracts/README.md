...
`contracts/audit`에는 Backend3가 제안한 Audit Runtime Event JSON Schema와 검증 Fixture가 있습니다. 아직 Contract Freeze 전이므로 문서와 Schema가 충돌하면 위 두 문서가 우선합니다. 합의 후에는 Schema 변경과 생산자·소비자 테스트를 같은 PR에서 갱신합니다.
