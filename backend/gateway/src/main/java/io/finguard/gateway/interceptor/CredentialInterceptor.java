...
public class CredentialInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (identity.isEmpty()) {
            return denyUnauthenticated(response, requestId);
        }
    ...
    private boolean denyUnauthenticated(HttpServletResponse response, String requestId) {
        log.warn("Credential verification failed requestId={}", requestId);
        safeRecordAuthFailure(requestId);
        response.setStatus(HttpStatus.UNAUTHORIZED.value());
        return false;
    }

    private void safeRecordAuthFailure(String requestId) {
        try {
            coreClient.recordAuthFailure(requestId, REASON_AUTH_FAILED);
        } catch (Exception e) {
