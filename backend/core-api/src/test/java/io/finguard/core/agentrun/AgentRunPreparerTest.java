...
class AgentRunPreparerTest {
    ...
    private static final RequestTrace TRACE = new RequestTrace("REQ-001", null);
    ...
    void callsTheDetectorWhenNoSnapshotExists() {
        ...
        when(client.evaluate(any(), any(), any(), any(), any())).thenReturn(Optional.of(low()));

        PreparedAgentRun prepared = preparer.prepare("대출심사를 진행해줘.", TRACE);
    ...
    void reusesAnEvaluatedSnapshotWithoutCallingTheDetector() {
        ...
        verify(client, never()).evaluate(any(), any(), any(), any(), any());
    }
    ...
    void reEvaluatesWhenOnlyANotEvaluatedSnapshotExists() {
        ...
        assertThat(prepared.evaluation()).contains(low());
        verify(client, times(1)).evaluate(any(), any(), any(), any(), any());
    }
    ...
    void survivesADetectorFailure() {
        when(snapshots.findByInputHashAndModelVersion(any(), eq(PromptRiskModel.CURRENT_VERSION)))
                .thenReturn(Optional.empty());
        when(client.evaluate(any(), any(), any(), any(), any())).thenReturn(Optional.empty());
    ...
    void derivesTheSameHashForTheSameTextButAFreshRunIdEachTime() {
        ...
        PreparedAgentRun first = preparer.prepare("같은 문장", TRACE);
        PreparedAgentRun second = preparer.prepare("같은 문장", TRACE);
