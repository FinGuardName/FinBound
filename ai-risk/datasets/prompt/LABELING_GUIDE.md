...
`groupId`는 Split 이름이나 언어가 아니라 **기본 의미와 요청 의도**를 기준으로 정합니다. 같은
공격 문구를 인용해 같은 분석·설명 작업을 요청한다면 문서, 교육, 로그처럼 외부 문맥이 다르거나
한국어·영어·혼합어로 표현되어도 같은 Group입니다. 반면 같은 공격 유형을 다루더라도 요청 목적과
입력 변형 방식이 독립적이면 다른 Group일 수 있습니다. Group을 정한 뒤에만 Group 전체를 하나의
Split에 배치합니다.

같은 작성 Template에서 공격 문구나 대상만 바꾼 문장도 독립 표본으로 취급하지 않습니다. 예를 들어
인용문을 실행하지 않고 분류하는 요청, 띄어쓰기 난독화 문구를 분석하는 요청처럼 요청 골격이 같다면
공격 유형이 달라도 같은 의미군으로 묶어 하나의 Split에만 배치합니다.
