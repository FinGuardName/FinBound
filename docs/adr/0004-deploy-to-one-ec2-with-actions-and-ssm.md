...
- **인스턴스 크기 — 재고 정했다. t3.medium(4GiB)이면 된다.**
  ...
  **한계:** 60건을 순차로 보냈다. 동시 요청이면 JVM 힙이 더 오른다. 브라우저 E2E는 돌리지 않았다.
  ...
  ### 🔴 이 측정을 잘못 읽었다 — 실제 배포에서 드러났다

  **`ai-risk` 904 MiB 는 "필요한 양"이 아니라 "한도가 허락한 최대치"였다.** 1GiB 한도의 88%에
  붙어 있던 것을 사용량으로 읽었는데, 한도가 수요를 누르고 있던 것이었다.

  EC2 배포에서 그대로 터졌다. `AI_RISK_MEM_LIMIT=1280m` 을 주고도 **OOM 으로 122번 재시작**했다.
  호스트에는 2.9GiB 가 남아 있었으므로 호스트 부족이 아니라 컨테이너 한도 문제였다.

  ```
  OOM=true   restarts=122   memlimit=1342177280(1280m)   host available=2944 MiB
  ```

  로그가 정황을 보여준다 — 기동은 매번 성공(`Application startup complete`)하는데 곧바로 죽고,
  헬스체크 `/ready` 는 2초 안에 응답하지 못한다. **`/ready` 가 모델을 실제로 적재하면서 한도를
  넘기는 것으로 보인다.**

  **`2g` 로 올려서 해결됐다.** `.env.example` 의 기본값도 2g 로 바꿨다.

  **교훈: 한도에 붙어 있는 사용량을 수요로 읽지 않는다.** 88% 는 "여유 있다" 가 아니라
  "눌려 있다" 는 신호였다.

- **swap 은 이 문제의 해법이 아니다.** 죽은 이유가 호스트 메모리 부족이 아니라 cgroup 한도이고,
  swap 을 켜면 ML 모델을 디스크로 밀어내 시연이 견딜 수 없이 느려진다. 한도를 올리는 것이 맞다.
