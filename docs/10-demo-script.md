...
이 대본은 PR #123의 카드 UI·정상 기본 문구 수정과 PR #128의 행동 위험 누적 기준을
함께 반영한 `develop` 커밋 `8da6f4c`를 기준으로 한다. 아래 검증을 통과한 빌드에서 사용한다. 기존 배포 주소를 사용할
때에도 같은 화면과 모델 버전인지 확인한다. 이 문서가 바뀌었다고 배포가 갱신되지는 않는다.

검증 기록: 2026-09-07 KST, `prompt-guard-6`, [#123·#128 머지 후 실제 8개 서비스 E2E 결과](https://github.com/FinGuardName/FinBound/actions/runs/34044157774).
정상 카드 세 개의 LOW·ALLOW와 별도 합성 완료 이력 20건을 사용한 행동 차단이 이 실행에 포함된다.
...
#123·#128을 함께 반영한 Real E2E에서 세 정상 카드가 모두 `promptRiskLevel=LOW`, `decision=ALLOW`,
...
기본 문구를 검증된 표현으로 바꾼 것은 시연 기본값의 오탐을 피하는 조치다. Prompt 모델·임계값·
규칙은 바꾸지 않았으며, #128도 행동 위험의 누적 기준만 조정했다. 한국어 정상 문구나 고객 ID가 포함된 문장에도 오탐이 있고,
