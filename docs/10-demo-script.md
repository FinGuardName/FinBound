...
이 대본은 PR #123의 카드 UI·정상 기본 문구, PR #128의 행동 위험 누적 기준,
PR #129의 `prompt-guard-7` 탐지 보완을 함께 반영한 빌드를 기준으로 한다.
아래 검증을 통과한 빌드에서 사용한다. 기존 배포 주소를 사용할
때에도 같은 화면과 모델 버전인지 확인한다. 이 문서가 바뀌었다고 배포가 갱신되지는 않는다.

검증 기록: 2026-09-07 KST, PR #129 커밋 `5b73c61`, `prompt-guard-7`,
[실제 8개 서비스 E2E 결과](https://github.com/FinGuardName/FinBound/actions/runs/34048601133).
정상 카드 세 개의 LOW·ALLOW, 공격 카드 세 개의 CRITICAL·BLOCK·금융시스템 미도달,
별도 합성 완료 이력 20건을 사용한 행동 차단이 이 실행에 포함된다.
이는 CI의 격리 환경 검증이며 운영 사이트에 같은 버전이 배포됐다는 뜻은 아니다.
...
#123·#128·#129를 함께 반영한 Real E2E에서 세 정상 카드가 모두 `promptRiskLevel=LOW`, `decision=ALLOW`,
...
기본 문구를 실제 `prompt-guard-7` ONNX 모델로 평가한 값은 다음과 같다.
...
AI Risk와 Core API는 함께 갱신해야 한다. 두 서비스의 Prompt 모델 계약을 `prompt-guard-7`로
맞춰 이전 `prompt-guard-6` Snapshot 대신 새 규칙으로 평가한 Snapshot을 사용한다.
새 실행의 감사 기록에서도 `promptModelVersion=prompt-guard-7`인지 확인한다.
...
```text
PASS: isolated behavior demo: 20 synthetic events, AI CRITICAL, OPA BLOCK, BEHAVIOR_ANOMALY
```
...
한도 재심사의 **심사 기준 무시 지시 차단** 카드와 보완서류의 **보완서류 지시 변조 차단**
카드도 기본 문구로 같은 조건을 검증한다. 세 공격 카드의 E2E는
`promptRiskLevel=CRITICAL`, `reasonCodes=[PROMPT_INJECTION]`,
`downstreamReached=false`, `responseReleased=false`를 모두 확인한다.
한도 재심사 문구는 #129에서 추가한 `POLICY_BYPASS` 보조 규칙과 모델 근거가 결합해
CRITICAL로 평가된다. 문구만 바꿔 탐지가 쉬운 예시로 대체한 수정은 아니다.
...
검증은 고정 시각 `2026-08-17T23:00:00Z`와 2초 간격의 과거 완료 호출 20건을 사용한다.
최근 5분의 **완료 이력 20건 + 현재 21번째 요청**을 AI에 전달한다. 현재 요청은 완료 이력에 포함하지 않는다.
...
#128 이후 최근 5분의 유효 완료 이력이 5건 미만이면 `COLD_START·LOW`이고, 5건부터는
모델을 평가하되 20건 미만에서는 최대 `ALERT`다. 현재 모델의 CRITICAL 임계값은 `1.0`이며,
이 초기 구간의 위험 점수 상한은 `0.9999`라 감사 DB의 소수점 4자리 저장 후에도 임계값 미만이다.
완료 이력 20건 이상부터 모델 점수가 CRITICAL 임계값을 충족하면 행동 차단 신호가 가능하다.
Scope·Prompt Risk·Gateway Hard Limit은 이 이력 조건과 별도로 계속 적용된다.
...
이 합성 패턴은 21번째 요청에서 차단을 검증하지만, 모든 21번째 요청이 차단되는 것은 아니다.
또한 최초 20회에 한 번만 적용되는 유예가 아니라 최근 5분 완료 이력을 매 요청마다 센다.
실제 행동 판정은 최근 5분 이력·요청 간격·
...
정상 기본 문구 수정(#123)은 시연 기본값의 오탐을 피하는 조치이고, #128은 행동 위험의
누적 기준을 조정했다. #129는 심사 기준·보호 기준·접근 제한을 무시한 뒤 조회/실행하라는
표현을 Prompt 보조 규칙에 추가했다. 모델 가중치와 전체 차단 임계값은 변경하지 않았으며,
규칙 단독으로는 ALERT이고 충분한 모델 근거가 함께 있어야 CRITICAL이다.
기본 공격 카드의 미탐을 보완했지만, 한국어 표현 변형·고객 ID 포함 문장·부정/설명 문맥의
오탐과 미탐은 남아 있다. 자유 입력을 막거나 모든 입력의 탐지 성공을 보장하지 않는다.

| 과거 관측 사례 (`prompt-guard-6`) | 당시 결과와 해석 |
|---|---|
...
위 표는 이전 모델 버전에서 관측한 기록이다. `prompt-guard-7`에서 한도 재심사의 같은
기본 공격 예시는 `POLICY_BYPASS`가 매칭되고 CRITICAL 0.9로 평가되며,
실제 Core·AI·OPA E2E에서 `PROMPT_INJECTION` 단독 사유로 차단된다.
어떤 버전이든 `BEHAVIOR_ANOMALY`만으로 막힌 결과는 프롬프트 탐지 성공이 아니다.
정상 3건·공격 3건의 검증을 운영의 9개 카드 조합 전체 또는 임의 자유 입력의 탐지 성능으로
확대해 말하지 않는다. 배포 후에는 실제 버전에서 9개 조합을 다시 확인한다.
